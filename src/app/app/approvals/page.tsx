...
export default async function ApprovalsPage() {
  ...
  const items: ApprovalItem[] = stored
    ? (stored as StoredApproval[]).map((approval) => {
        const runRelation = approval.workflow_runs;
        const run = Array.isArray(runRelation) ? runRelation[0] : runRelation;
        const workflow = Array.isArray(run?.workflows) ? run?.workflows[0]?.name : run?.workflows?.name;
        return {
          id: approval.id,
          workflow: workflow ?? "Workflow",
          prompt: approval.prompt,
          preview: approval.preview as ApprovalItem["preview"],
    ...
    : approvals.map((approval, index) => ({
        ...
        prompt: approval.action,
        preview: approval.body,
        requestedAt: new Date(Date.UTC(2026, 7, 31, 8 - index)).toISOString(),
      }));
  return (
    <main className="app-page approvals-page">
      <header className="page-title-row">
        <div>
          <span className="page-kicker">Human in the loop</span>
          <h1>Approvals</h1>
          <p>Inspect the exact action before Automata continues.</p>
        </div>
      </header>
      <div className="approval-layout">
        <ApprovalInbox initial={items} />
        <aside className="approval-policy">
          <h2>Approval policy</h2>
          <p>AI-created workflows pause before external writes unless the author explicitly requests unattended execution.</p>
          <dl>
            <div>
            ...
            <div>
              <dt>Public publishing</dt>
              <dd>Always ask</dd>
            </div>
            <div>
              <dt>Internal data sync</dt>
              <dd>Workflow setting</dd>
            </div>
          </dl>
        </aside>
      </div>
