...
import { outEdges, TRIGGER_TYPES, type StepType, type WorkflowGraph, type WorkflowStep } from "@/lib/workflows/types";
...
function makeFlow(graph: WorkflowGraph) {
  const depth = new Map<string, number>([[graph.start, 0]]);
  const queue = [graph.start];
  const visited = new Set<string>();
  while (queue.length) {
    const id = queue.shift();
    if (!id || !graph.steps[id] || visited.has(id)) continue;
    visited.add(id);
    const nextDepth = (depth.get(id) ?? 0) + 1;
    for (const target of outEdges(graph.steps[id])) {
      if (!graph.steps[target]) continue;
      if (!depth.has(target) || nextDepth < (depth.get(target) ?? Infinity)) depth.set(target, nextDepth);
      queue.push(target);
    }
  }
  for (const id of Object.keys(graph.steps)) if (!depth.has(id)) depth.set(id, Math.max(0, ...depth.values()) + 1);
  ...
  for (const [level, ids] of levels) {
    ids.forEach((id, index) => {
      const totalHeight = (ids.length - 1) * 150;
      nodes.push({ id, type: "module", position: { x: 90 + level * 230, y: 250 - totalHeight / 2 + index * 150 }, data: { step: graph.steps[id], kind: kindForStep(graph.steps[id]) }, draggable: false });
    });
  }
  const edges: Edge[] = Object.values(graph.steps).flatMap((step) => outEdges(step).map((target, index) => ({
    id: `${step.id}-${target}-${index}`,
    source: step.id,
    target,
    type: "flow",
    data: { label: edgeLabel(step, target) },
    markerEnd: { type: MarkerType.ArrowClosed, width: 12, height: 12, color: "#cbcdd9" },
  })));
  return { nodes, edges };
}

function edgeLabel(step: WorkflowStep, target: string) {
  if (step.onApprove === target) return "Approved";
  if (step.onReject === target) return "Rejected";
  if (step.onFalse === target) return "No";
  const named = Object.entries(step.cases ?? {}).find(([, value]) => value === target);
  return named?.[0] ?? "";
}
...
export function WorkflowBuilder({ isNew = false, workflowId, workflowName, initialGraph, initiallyPublished }: { isNew?: boolean; workflowId?: string; workflowName?: string; initialGraph?: WorkflowGraph; initiallyPublished?: boolean }) {
  const router = useRouter();
  const [graph, setGraph] = useState(initialGraph ?? (isNew ? starterGraph : demoGraph));
  ...
  async function save() {
    ...
    try {
      if (isNew || !workflowId || workflowId === "new") {
        const response = await fetch("/api/workflows", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name, graph }) });
      ...
      const response = await fetch(`/api/workflows/${workflowId}`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ name, graph, changeSummary: "Updated in the visual editor" }) });
      const body = await response.json() as { error?: string };
      if (!response.ok) throw new Error(body.error ?? "Workflow could not be saved.");
      setDirty(false); setNotice("Draft saved.");
  ...
  async function askAutomata() {
    ...
    try {
      ...
      updateGraph(body.graph); setSelectedId(null); setAiPrompt(""); setNotice("The new draft is on the canvas. Review it, then save.");
    } catch (error) { setNotice(error instanceof Error ? error.message : "Automata could not apply that change."); }
    finally { setBusy(null); }
  }

  function addStep(item: (typeof palette)[number]) {
    ...
    const after = selectedId && graph.steps[selectedId] ? selectedId : graph.start;
    const previous = graph.steps[after];
    const newStep: WorkflowStep = { id, type: item.type, name: item.title, next: previous.next ?? null };
    if (item.type === "ai") newStep.instruction = "Describe what this step should produce.";
    if (item.type === "approval") { newStep.prompt = "Continue this run?"; newStep.onApprove = previous.next ?? null; newStep.onReject = null; newStep.next = null; }
    const updatedPrevious = { ...previous, next: id };
    updateGraph({ ...graph, steps: { ...graph.steps, [after]: updatedPrevious, [id]: newStep } });
  ...
  function deleteSelected() {
    ...
    const replacement = selected.next ?? selected.onApprove ?? null;
    delete steps[selected.id];
    for (const [id, step] of Object.entries(steps)) {
      const next = { ...step };
      if (next.next === selected.id) next.next = replacement;
      if (next.onFalse === selected.id) next.onFalse = replacement;
      if (next.onApprove === selected.id) next.onApprove = replacement;
      if (next.onReject === selected.id) next.onReject = replacement;
      steps[id] = next;
    }
    updateGraph({ ...graph, steps }); setSelectedId(null);
  }
  ...
  return <div className="builder-shell growth-editor">
    <header className="growth-editor-header">
      ...
      <div className="growth-editor-actions"><button className="editor-icon-button" aria-label="Undo" disabled><Undo2 size={15} /></button><button className="editor-icon-button" aria-label="Redo" disabled><Redo2 size={15} /></button><button className={`editor-ai-toggle ${aiOpen ? "active" : ""}`} onClick={() => setAiOpen((value) => !value)}><Sparkles size={15} />Ask Automata</button><button className="editor-save" onClick={() => void save()} disabled={busy !== null}>{busy === "save" ? <Loader2 className="spin" size={15} /> : <Save size={15} />}{dirty ? "Save" : "Saved"}</button><button className="editor-run" onClick={() => { setNotice(""); setRunOpen(true); }}><Play size={14} fill="currentColor" />Run once</button><button className="editor-publish" onClick={() => void publish()} disabled={busy !== null}>{busy === "publish" ? <Loader2 className="spin" size={14} /> : published ? <Check size={14} /> : null}{published ? "Published" : "Publish"}<ChevronDown size={13} /></button><button className="editor-icon-button" aria-label="More actions"><MoreHorizontal size={16} /></button></div>
    </header>

    {notice ? <div className="editor-notice" role="status"><span>{notice}</span><button onClick={() => setNotice("")} aria-label="Dismiss"><X size={13} /></button></div> : null}

    <div className="growth-editor-body">
      {aiOpen ? <aside className="editor-ai-panel"><header><div><span><Sparkles size={15} /></span><div><b>Ask Automata</b><small>Change this workflow in plain English</small></div></div><button onClick={() => setAiOpen(false)} aria-label="Close assistant"><X size={15} /></button></header><div className="editor-ai-empty"><span><Sparkles size={20} /></span><h2>What should change?</h2><p>Describe the outcome. Automata will redraw the draft without publishing it.</p></div><div className="editor-ai-prompts"><button onClick={() => setAiPrompt("Add a human approval before the final external action")}>Add approval before publishing</button><button onClick={() => setAiPrompt("Send failures to Slack with the error details")}>Alert Slack when it fails</button></div><div className="editor-ai-composer"><textarea rows={4} value={aiPrompt} onChange={(event) => setAiPrompt(event.target.value)} placeholder="Add a filter after the HubSpot step…" /><button disabled={!aiPrompt.trim() || busy !== null} onClick={() => void askAutomata()}>{busy === "ai" ? <Loader2 className="spin" size={15} /> : <ArrowRightIcon />}</button></div></aside> : null}

      <main className="workflow-canvas growth-canvas" data-testid="workflow-canvas">
        <ReactFlowProvider><WorkflowWhiteboard nodes={flow.nodes} edges={flow.edges} selectedId={selectedId} onSelect={(id) => { setSelectedId(id); setLibraryOpen(false); }} /></ReactFlowProvider>
      ...
      {selected ? <Inspector step={selected} isStart={selected.id === graph.start} onPatch={patchSelected} onDelete={deleteSelected} onClose={() => setSelectedId(null)} /> : null}
    </div>
...
function WorkflowWhiteboard({ nodes: initialNodes, edges: initialEdges, selectedId, onSelect }: { nodes: ModuleFlowNode[]; edges: Edge[]; selectedId: string | null; onSelect: (id: string) => void }) {
  const [nodes, setNodes, onNodesChange] = useNodesState(initialNodes);
  const [edges, setEdges, onEdgesChange] = useEdgesState(initialEdges);
  const { fitView, zoomIn, zoomOut } = useReactFlow();
  useEffect(() => { setNodes(initialNodes); setEdges(initialEdges); window.setTimeout(() => void fitView({ padding: .22, duration: 220 }), 20); }, [initialNodes, initialEdges, setNodes, setEdges, fitView]);
  return <ReactFlow nodes={nodes.map((node) => ({ ...node, selected: node.id === selectedId }))} edges={edges} nodeTypes={nodeTypes} edgeTypes={edgeTypes} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onNodeClick={(_, node) => onSelect(node.id)} fitView fitViewOptions={{ padding: .22, minZoom: .45, maxZoom: 1 }} minZoom={.25} maxZoom={1.6} nodesConnectable={false} proOptions={{ hideAttribution: false }}>
    <Background variant={BackgroundVariant.Dots} gap={22} size={1.35} color="#cbcdd9" />
    <Panel position="bottom-left" className="canvas-controls-new"><button onClick={() => void zoomIn({ duration: 150 })} aria-label="Zoom in"><ZoomIn size={14} /></button><button onClick={() => void zoomOut({ duration: 150 })} aria-label="Zoom out"><ZoomOut size={14} /></button><button onClick={() => void fitView({ padding: .22, duration: 240 })} aria-label="Fit to view"><Maximize size={14} /></button></Panel>
  </ReactFlow>;
}
...
function ModuleNode({ data, selected }: NodeProps<ModuleFlowNode>) {
  ...
  return <div className="growth-node-wrap">
    <Handle type="target" position={Position.Left} className="growth-node-handle" />
    ...
    <Handle type="source" position={Position.Right} className="growth-node-handle" />
  </div>;
}
...
function Inspector({ step, isStart, onPatch, onDelete, onClose }: { step: WorkflowStep; isStart: boolean; onPatch: (patch: Partial<WorkflowStep>) => void; onDelete: () => void; onClose: () => void }) {
  const kind = kindForStep(step);
  return <aside className="growth-inspector"><header><div><small>{step.app ? INTEGRATION_BY_SLUG.get(step.app)?.name ?? step.app : kind === "trigger" ? "Trigger" : kind}</small><h2>{step.name}</h2></div><button onClick={onClose} aria-label="Close inspector"><X size={15} /></button></header><div className="growth-inspector-summary"><StepArt step={step} kind={kind} /><span><b>{step.app ? "Connected account" : step.type.replaceAll("_", " ")}</b><small>{step.operation === "write" ? "External write · approval protected" : step.app ? "Acme workspace" : "Built into Automata"}</small></span><button>Change</button></div><div className="growth-inspector-section"><span>Configuration</span><label>Step name<input value={step.name} onChange={(event) => onPatch({ name: event.target.value })} /></label>{kind === "ai" ? <><label>Instruction<textarea rows={7} value={String(step.instruction ?? "")} onChange={(event) => onPatch({ instruction: event.target.value })} /></label><label>Output<select defaultValue="structured"><option value="structured">Structured data</option><option value="text">Plain text</option></select></label></> : kind === "trigger" ? <><label>Schedule or event<input value={String(step.cron ?? step.event ?? "On demand")} onChange={(event) => onPatch(step.type === "schedule_trigger" ? { cron: event.target.value } : { event: event.target.value })} /></label><label>Timezone<select defaultValue="Asia/Kolkata"><option>Asia/Kolkata</option><option>UTC</option></select></label></> : kind === "approval" ? <label>Decision prompt<textarea rows={4} value={String(step.prompt ?? "")} onChange={(event) => onPatch({ prompt: event.target.value })} /></label> : <><label>Connected account<select defaultValue="acme"><option value="acme">Acme workspace</option></select></label><label>Action<input value={String(step.action ?? step.method ?? step.name)} onChange={(event) => onPatch({ action: event.target.value })} /></label></>}</div><div className="growth-inspector-section"><span>Failure handling</span><label className="inspector-toggle"><span><b>Stop the run</b><small>Keep the error visible in history</small></span><input type="checkbox" defaultChecked /></label></div><footer>{!isStart ? <button className="inspector-delete" onClick={onDelete}><Trash2 size={14} />Delete step</button> : <span />}<button className="inspector-done" onClick={onClose}>Done</button></footer></aside>;
}
