...
export function TemplateGrid({ templates }: { templates: TemplateCard[] }) {
  ...
  return <>{notice ? <p className="auth-alert preview integration-notice" role="status">{notice}</p> : null}<section className="app-template-grid">{templates.map((template) => <article className="app-template-card" key={template.id}><div className="app-template-meta"><span>{template.category ?? "Workflow"}</span><small><Clock3 size={11} />{template.setupMinutes ?? 4} min setup</small></div><div className="app-template-apps">{template.appBadges.map((app) => <ServiceIcon key={app.slug} slug={app.slug} />)}</div><h2>{template.name}</h2><p>{template.description}</p><footer><span>{Object.keys(template.graph.steps).length} modules</span><button disabled={pending === template.id} onClick={() => instantiateTemplate(template.id)}>{pending === template.id ? "Creating…" : "Use template"}<ArrowRight size={13} /></button></footer></article>)}</section></>;
}
